...
def build_norm_layer(cfg, num_features, postfix=""):
    # patching nework build
    # it default to batch sync
    network = nn.BatchNorm2d(num_features=num_features)
    return "", network
...
def trunc_normal_(
    tensor, mean: float = 0.0, std: float = 1.0, a: float = -2.0, b: float = 2.0
):
...
class UAttention(BaseModule):
    def __init__(self, dim):
        ...
        self.down1_1 = nn.AvgPool2d(reduction[0])
        self.down2_0 = UAttentionLayer(dim)
        self.down2_1 = nn.AvgPool2d(reduction[1])
    ...
    def forward(self, x):
        ...
        attn = torch.concat(
            [attn_0, attn_1, attn_2], axis=1
        )  # attn_0+attn_1#attn_0 + attn_1 + attn_2
