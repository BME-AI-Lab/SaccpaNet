...
from lib.modules.core.loss import JointsFocalLoss
...
class MyLightningModule(ClassificationModule):
    def __init__(self):
        super().__init__()
        self.conv = nn.Conv2d(in_channels=19,out_channels=3,kernel_size=3,padding=1).cuda()
        self.init_conv = nn.Conv2d(in_channels=1,out_channels=3,kernel_size=1,padding=1).cuda()
        self.fix_padding_conv =nn.Conv2d(in_channels=18,out_channels=18,kernel_size=3,padding="valid").cuda()
        self.net = models.segmentation.deeplabv3_resnet50(num_classes=18,aux_loss=False)
        self.classify_net = eca_resnet50(num_classes=7)#(num_classes = 7)
        self.joint_loss = JointsFocalLoss(use_target_weight=True)
