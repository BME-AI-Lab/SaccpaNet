...
class UAttentionLayer(BaseModule):
    def __init__(self, dim):
        ...
        #self.conv1x1 = nn.Conv2d(dim, dim, (1, 1), padding="same", groups=dim//5) # v3+1x1
    ...
    def forward(self,x):
        ...
        #attn = self.conv1x1(attn)# v3+1x1
        
        #attn = attn + attn_0 + attn_1 + attn_2
        return attn 

class UAttention(BaseModule):
    def __init__(self, dim):
        super().__init__()
        image_size = 512
        reduction = 2,2
...
class MSCAU(BaseModule):
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(MSCAU, self).__init__(init_cfg=init_cfg)
